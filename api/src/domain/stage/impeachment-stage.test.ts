...
describe('Estágio de Cassação', () => {
  it('deve escolher o alvo', () => {
    const stage = new ImpeachmentStage(new Player('p1', 'p1', Role.MODERADO));

    stage.chooseTarget(new Player('p2', 'p2', Role.MODERADO));
  ...
  it('deve permitir pular votação um conservador já tiver sido cassado, o alvo for um conservador e o radical ainda não tenha sido cassado', () => {
    const stage = new ImpeachmentStage(
      new Player('p1', 'p1', Role.MODERADO),
      true,
    );

    stage.chooseTarget(new Player('p2', 'p2', Role.CONSERVADOR));

...
const CPIStageSelectRapporteur = () => {
  const { player } = usePlayerContext();
  const { lobby } = useLobbyContext();
  const { cpiStageSelectRapporteur } = useLobbySocketContext();
    ...
    return (
      <>
        <h2 className="scroll-m-20 border-b pb-2 text-3xl font-semibold tracking-tight">
          {roundPrefix} -  Escolha do Relator da CPI
        </h2>
        ...
        <Button className="flex" variant="default" disabled>
          ...
          Aguarde a escolha do relator da CPI pelo presidente.
        </Button>
      </>
    );
  }
  ...
  return (
    <>
      ...
      <div className="text-sm max-w-lg text-muted-foreground flex flex-col gap-2">
        <p className="text-gray-700">
          Selecione um relator para a CPI do próximo governo. Ele sabrá
              ...
              <TooltipTrigger asChild>
                <div>
                  <Button
                    ...
                    variant="outline"
                    onClick={() => cpiStageSelectRapporteur(player.id)}
                    className="w-full"
                  >
                    {player.name}
                  </Button>
...
export default CPIStageSelectRapporteur;

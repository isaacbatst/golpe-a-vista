  ...
  return (
    <>
      ...
      <p className="text-muted-foreground text-center max-w-md">
        {stage.voting?.result ? (
          "O jogador perdeu os direitos políticos e não poderá mais participar do governo."
        ) : (
          <>
            Como os ânimos não se acalmaram, uma{" "}
            <strong>lei conservadora foi aprovada</strong> na marra.
          </>
        )}
      </p>
      {player.isPresident && (
        <Button onClick={() => impeachmentStageAdvanceStage()}>
          <ChevronsRight />
          Avançar
        </Button>
      )}
